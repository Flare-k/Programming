...
public:
    ...
    int ReplaceItem(T& data); // 입력된 data와 Primary key와 동일한 기록을 찾아서 List의 해당 기록을 입력된 data로 치환한다.
...
template<typename T>
int UnsortedList<T>::ReplaceItem(T& data){//clear
    for(int i = 0; i < m_Length; i++){
        if(data == m_Array[i]){
            m_Array[i] = data;
            return 1;
