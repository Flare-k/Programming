...
void Application::RemoveItem()
{
    ...
    Tree.Retrieve(item, found);        // RetrieveItem 함수를 통해서 Tree에서 검색
    if (found == true)                    // Tree에 item이 있으면
    {
        Tree.Delete(item);            // DeleteItem함수를 통해서 Tree에서 item값에 해당하는 노드를 삭제함
...
void Application::SearchItem()
{
    ...
    Tree.Retrieve(item, found);                    // RetrieveItem 함수를 통해 Tree에서 검색
